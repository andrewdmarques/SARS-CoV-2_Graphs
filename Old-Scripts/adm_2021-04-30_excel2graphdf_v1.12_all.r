...
graph_sars <- function(df_original, lineage_important, title, rationale_list, graph_type, date_start, date_stop, date_label, time_bin, breaks_num, bar_width){
...
# Make a dataframe of the specified samples
...
#Determine the max value for the y axis in the genomes sequenced graph
seqs_per_bin <- data4 %>%
  group_by(date) %>%
  summarise(total_seqs = sum(Lineage_count, na.rm = TRUE))
y_max <- max(seqs_per_bin$total_seqs)
...
p2 <- data4 %>%
  ...
  labs(x = "Date", y = "Genomes Sequenced") +
  scale_x_date(date_labels = date_label, breaks = breaks_pretty(breaks_num)) +
  scale_y_continuous(trans = 'log2', limits = c(0, y_max)) + 
  ylim(0,y_max)
...
# breaks_num <- 6
# bar_width <- 10
# graph_sars(df_original, lineage_important, title, rationale_list, graph_type, date_start, date_stop, date_label, time_bin, breaks_num, bar_width)
# 
