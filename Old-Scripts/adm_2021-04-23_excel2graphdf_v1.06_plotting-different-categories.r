...
lineage_important <- c("B.1.1.7","P.1", "B.1.351", "B.1.427", "B.1.429", "B.1.243", "B.1.2", "B.1.526", "B.1.526.1", "B.1.526.2", "R.1")
...
df9 %>%
  ...
  mutate(lineage_cat = factor(lineage),
         lineage_cat = forcats::fct_other(f = lineage_cat, drop = lineage_drop, other_level = "Other")) %>%
...
p1 <- ggplot(data3, aes(fill=lineage_cat, y=lineage_cat_perc, x=date)) + 
   ...
   ggtitle("Philadelphia Hospitalized SARS-CoV-2 Variants") + 
